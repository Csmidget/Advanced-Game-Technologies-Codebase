...
namespace NCL {
	namespace CSC8503 {
		...
		public:
			...
			GameObject* CreateCapsule(Vector3 position, Quaternion orientation, float halfHeight, float radius, float inverseMass = 10.0f, bool isStatic = false);
			GameObject* CreateSphere(Vector3 position, float radius, float inverseMass = 10.0f, bool isStatic = false);
